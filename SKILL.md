...
重构后的代码必须满足以下四条（详细规则见 [conventions.md](conventions.md)）：
...
4. **接口文档规范**：对外接口的 Swagger/OpenAPI 注解必须齐套——Controller 类有 `@Api`/`@Tag`，接口方法有 `@ApiOperation`/`@Operation`，每个请求参数和 DTO 字段有含义说明与示例值，注解内容与实际行为一致。
...
| 5 | 分层改造：补齐 service 接口 + serviceImpl，Controller 业务逻辑下沉；同批补齐/修正 Swagger 注解 | 中 |
